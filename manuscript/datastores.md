...
(define gd (lambda (data)
    ...
    (let ((jd (json->listvals data)))
      ...
      (define gg1 (lambda (jd) (map list (car jd))))
      (define gg2 (lambda (jd) (map list (car jd) (cadr jd))))
      (define gg3 (lambda (jd) (map list (car jd) (cadr jd) (caddr jd))))
      (define gg4 (lambda (jd) (map list (car jd) (cadr jd) (caddr jd) (cadddr jd))))
      ...
      (case (length (json->listvals data))
        ...
        [else (error "Function sparql-dbpedia accepts sparql queries with 1 to 4 variables")]))))

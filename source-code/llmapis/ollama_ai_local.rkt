...
(define (helper prompt . model-name)
  ...
  (let* ((model
          (if (equal? model-name '(())) "mistral:v0.3" (first (first model-name))))
         (prompt-data
          (string-join
           (list
            (string-append
...
;;(displayln (question-ollama-ai-local "Mary is 30 and Harry is 25. Who is older and by how much?" "llama3:latest"))

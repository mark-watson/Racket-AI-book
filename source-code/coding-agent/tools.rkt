...
;; ---------------------------------------------------------------------------
;; Interrupt helpers (avoid circular require at load time)

(define (task-interrupted?)
  (with-handlers ([exn:fail? (lambda (_) #f)])
    ((dynamic-require "fireworks-ai.rkt" 'task-interrupted?))))
...
;; propose_edit needs approval.rkt helpers -- loaded lazily to keep startup cheap
(define (tool-propose-edit path old new)
  (define unified-diff
    (dynamic-require "approval.rkt" 'unified-diff))
  (define print-colored-diff
    (dynamic-require "approval.rkt" 'print-colored-diff))
  (define prompt-yes-no-skip
    (dynamic-require "approval.rkt" 'prompt-yes-no-skip))
  (define prompt-reason
    (dynamic-require "approval.rkt" 'prompt-reason))
  (define task-interrupted?
    (dynamic-require "fireworks-ai.rkt" 'task-interrupted?))
...
(define (register-all)
  ...
  (define-tool
    "run_shell"
    (list (list "command" "string" "Shell command. Only whitelisted commands may run: make, ls, pwd, cat."))
